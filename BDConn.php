...
class BDConn {
    ...
    public function insertDBEvent($desc,$tipo,$timestamp,$pos,$lat,$long,$profundidad,$temp_agua,$sal,$fluor,$conductividad,$temp_aire,$humedad,$pres_atmos,$vel_med_viento,$fecha_fin,$instrument) {
	$sql = "INSERT INTO eventos (Descripcion, Tipo, Timestamp, Pos, Latitud, Longitud, Profundidad, Temp_agua, Sal, Fluor, Conductividad, Temp_aire, Humedad, Pres_atmos, Vel_med_viento, Fecha_fin, Instrument)"
	. "VALUES ('$desc','$tipo','$timestamp','$pos','$lat','$long','$profundidad','$temp_agua','$sal','$fluor','$conductividad','$temp_aire','$humedad','$pres_atmos','$vel_med_viento',".($fecha_fin=='null' ? $fecha_fin : "'$fecha_fin'").",'$instrument')";
    ...
    public function updateEvent($id,$desc,$tipo,$timestamp,$pos,$lat,$long,$profundidad,$temp_agua,$sal,$fluor,$conductividad,$temp_aire,$humedad,$pres_atmos,$vel_med_viento,$fecha_fin,$instrument) {
        $sql= "UPDATE eventos SET Descripcion='$desc', Tipo='$tipo', Timestamp='$timestamp', Pos='$pos', Latitud='$lat', Longitud='$long', Profundidad=$profundidad, Temp_agua=$temp_agua, Sal=$sal, Fluor=$fluor, Conductividad=$conductividad, Temp_aire=$temp_aire, Humedad=$humedad, Pres_atmos=$pres_atmos, Vel_med_viento=$vel_med_viento, Fecha_fin=".($fecha_fin=='null' ? $fecha_fin : "'$fecha_fin'").", Instrument='$instrument' "
                . "WHERE ID = $id";
        mysqli_query($this->conexion, $sql) or die(mysqli_error($this->conexion));
    }
...
//    }
   ...
   public function getSadoLat($momento, $intervalo){
    //$sql = "select longitud, latitud from posicion order by fecha desc limit 1";
        $sql = "select latitud from posicion where fecha between '$intervalo' and '$momento' order by fecha desc limit 1;";
        $query =  mysqli_query($this->conexion, $sql) or die(mysqli_error($this->conexion));
        $arQu = mysqli_fetch_array($query,MYSQLI_ASSOC);
        $lat = $arQu['latitud'];
        if($lat == ""){
    ...
    public function getSadoLong($momento, $intervalo){
        //$sql = "select longitud, latitud from posicion order by fecha desc limit 1";
            $sql = "select longitud from posicion where fecha between '$intervalo' and '$momento' order by fecha desc limit 1;";
            $query =  mysqli_query($this->conexion, $sql) or die(mysqli_error($this->conexion));
            $arQu = mysqli_fetch_array($query,MYSQLI_ASSOC);
            $long = $arQu['longitud'];
            if($long == ""){
